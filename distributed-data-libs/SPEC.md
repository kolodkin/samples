...
### No-materialize companions (Column multiply (no mat) / Filter rows (no mat) / Sort (no mat))

Three more ops run the same large-result operators (`Column multiply` / `Filter rows` / `Sort`) but **discard** the result instead of keeping it in-engine: the full pipeline runs over all 10M rows server-side, then nothing is materialized or transported back. Paired with the standard full-materialize rows, the contrast isolates the cost of *retaining* the result (a written result table / cluster-memory cache) from the cost of the *compute* alone.

aaiclick can do this library-natively only as of **v0.0.16**, which added `Object.execute()` — it rebuilds the exact SELECT that `.data()` would issue, appends `FORMAT Null`, runs it, and returns a `QueryStats`. Before v0.0.16 the only no-materialize path was raw `ch_client.command("… FORMAT Null")`, which would have measured ClickHouse-the-engine rather than aaiclick-the-library; `execute()` keeps the comparison library-vs-library. Each framework's discard idiom:

- **aaiclick** — `obj.where(…).execute()` / `obj.view(order_by=…).execute()`; the multiply goes through `obj.with_columns({"product": Computed(…)}).execute()` because a bare operator's `.execute()` materializes first (`LazyOperator` has no pre-materialized table).
- **Spark** — `df.write.format("noop").mode("overwrite").save()`: the noop sink forces the full job with no cache, no collect, and nothing written.
- **Dask** — `ddf.map_partitions(len).compute().sum()`: forces the pipeline over all rows but reduces each partition to its length, so nothing persists in worker memory and only per-partition counts reach the driver. (For `Sort (no mat)` this measures the shuffle that defines the output partitions; query optimization may skip the order-only reordering that a true materialize would force.)
...
2. The runner inside the container loops through all 17 ops (11 standard + 3 paginated companions + 3 no-materialize companions) in a single long-lived process and writes `/data/results-<framework>.json`
...
- **No-materialize companions** — `Column multiply (no mat)` / `Filter rows (no mat)` / `Sort (no mat)` reuse the same View/operator expressions but call `Object.execute()` (new in aaiclick v0.0.16) instead of `.copy()`. `execute()` rebuilds the View's SELECT, appends `FORMAT Null`, and runs it — CH runs the full pipeline over all 10M rows and emits zero rows, so nothing is written to a result table or streamed back. This is the library-native version of the raw `FORMAT Null` the full-materialize note above rules out, so the no-materialize column stays library-vs-library too. Multiply routes through `with_columns({"product": Computed(…)}).execute()` because a bare `LazyOperator`'s `.execute()` would materialize a table first.
...
- **No-materialize companions** — `Column multiply (no mat)` / `Filter rows (no mat)` / `Sort (no mat)` use `df.write.format("noop").mode("overwrite").save()`. The noop sink forces the full job server-side with no cache, no `collect()`, and nothing written — the canonical Spark "compute only" pattern, matching aaiclick's `execute()`.
...
- **No-materialize companions** — `Column multiply (no mat)` / `Filter rows (no mat)` / `Sort (no mat)` use `ddf.map_partitions(len).compute().sum()` (the discard idiom the in-memory materialize above replaced for the standard rows): the pipeline runs over all rows but each partition reduces to its length, so nothing persists in worker memory and only per-partition counts return to the driver. Matches aaiclick's `execute()` and Spark's noop sink. For `Sort (no mat)` this captures the shuffle that defines the output partitions; query optimization may skip the within-partition reorder that a true materialize forces, so read it as a lower bound on Sort's no-materialize cost.
