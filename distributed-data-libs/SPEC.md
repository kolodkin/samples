...
- **Materialize without copy** — standard large-result ops (`Column multiply`, `Filter rows`, `Sort`) run the operation's `SELECT … FORMAT Null` via `ch_client.command`. `FORMAT Null` makes CH execute the full pipeline (project all 10M products / evaluate the filter / sort all 10M rows) and discard the serialized output — the exact analog of Spark's noop sink and Dask's `map_partitions(len)`. The earlier approach used the arithmetic operator and `.copy()`, but both **CREATE a new MergeTree table holding the full result** (aaiclick operators "create new tables with results" by design), so they charged these ops an extra ~10M-row server-side write that the Spark/Dask materialize paths never pay — an unfair penalty against aaiclick. `FORMAT Null` removes it and also stops leaking a result table per run.
