...
| PCL face scan    | `PointCloudLibrary/data` `biwi_face_database/model.pcd` | raw.githubusercontent (CORS)  |
| KITTI movie      | `kolodkin/pcl-viewer-kitti-movie` (HF dataset)        | HF resolve (CORS), Draco `.drc` |

### Per-scene normalization profiles
The two static scenes need different handling, so `viewer.js` keys a small
**profile** off the scene id. The KITTI clouds (city, movie) are z-up sensor frames
spread over a wide ground plane: rotate z-up → y-up, scale by a robust *horizontal*
(x,z) radius, and frame from the low forward-facing chase camera (the default
described above). The PCL **face scan** is a compact object already in a y-up frame
facing +Z (nose protruding toward +Z, crown at +Y, 100k points, no NaNs): it skips
the rotation, scales by a robust *3D* radius so the whole head fits the 45° FOV, and
is framed **front-on** (camera on +Z, lifted and nudged right for a near-frontal
portrait). Both profiles still center on the origin and normalize to `sceneRadius
0.5`, so the point-size slider and the height/distance color ramps work unchanged
across scenes (the face carries no `intensity` field, so that mode falls back to
flat).
...
city/face, `loadMovie` (DRACOLoader) for the movie. The movie **streams**:
...
citation per the BY + SA terms. The PCL face scan comes from the **BIWI Kinect Head
Pose Database** (ETH Zurich), which is free for **research/academic use** — not a
permissive license — and is attributed accordingly (Fanelli et al., *Random Forests
for Real Time 3D Face Analysis*, IJCV 2013), consistent with the project's existing
non-permissive KITTI clouds. It is hot-linked at runtime, not committed.
