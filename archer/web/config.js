...
export const CONFIG = {
  ...
  // length (1 → 2 → 2 → 3 → 3). Rounds are deliberately short: the late
  // arc adds waves but keeps each one smaller than the forest/desert
  // waves — its difficulty comes from the mix (ogres, skeleton pairs)
  ...
  stages: {
    ...
    iceberg: { speedMult: 1.1, dropMult: 1.25, healMult: 1.3,
      ...
      { goblin: 2, skeleton: 1 },
      { goblin: 2, skeleton: 1, ogre: 1 },
      { goblin: 3, skeleton: 1, ogre: 1 },
    ] },
    volcano: { speedMult: 1.15, dropMult: 1.25, healMult: 1.4,
               grant: { exploding: 20, freezing: 10 }, waves: [
      { goblin: 3, skeleton: 1 },
      { goblin: 2, skeleton: 2, ogre: 1 },
      { goblin: 2, skeleton: 1, ogre: 2 },
    ] },
  },
};
