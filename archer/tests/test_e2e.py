...
# is hittable (shared with the buried-peek test below). Both traps are
...
def _walker_escapes_trap(server_url, page, obstacles, spawn):
    """Boot at the pinned 60 Hz step, build the trap layout, and require
    the skeleton to come out hittable."""
    page.goto(server_url + BOOT)
    _wait_ready(page)
    page.evaluate("() => window.__ARCHER.setPlayerHp(10000)")
    page.evaluate(
        """(a) => {
          ...
          window.__ARCHER.setObstacles(a.obstacles);
          window.__ARCHER.spawnEnemy('skeleton', a.x, a.z);
        }""",
        {"obstacles": obstacles, "x": spawn[0], "z": spawn[1]},
    )
...
def test_skeleton_escapes_a_tree_pair_across_its_lane(server_url, page):
    ...
    _walker_escapes_trap(server_url, page, [
...
def test_skeleton_peek_is_never_buried_by_neighbor_trees(server_url, page):
    ...
    page.wait_for_function(EXPOSED_PREDICATE, timeout=30000)
...
def test_burning_lob_splits_into_a_volley(server_url, page):
    ...
    # Five fragments into bare ground resolve as ONE missed shot, advancing
    # the miss streak by one, not five: auto stays armed...
    page.wait_for_function("() => window.__ARCHER.state.arrowCount === 0", timeout=5000)
    assert page.evaluate("() => window.__ARCHER.state.selected") == "freezing"
    # ...until two more misses complete the streak of three.
    _miss(page)
    _miss(page)
